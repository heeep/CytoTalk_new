...
Let’s try reading in the folder.Due to file size constraints, we have placed a simplified example dataset called extdata in the inst folder of the CytoTalk package. This dataset is provided to help users understand how to use the read_matrix_folder function. It includes only the following two files:
```{r}
#> ── scRNAseq-data
#>    ├─ scRNAseq_Fibroblasts.csv
#>    ├─ scRNAseq_LuminalEpithelialCells.csv
...
``` r
dir_in <- system.file("extdata", package = "mycytotalk")
...
``` r
base_path <- mycytotalk::get_example_data()
fpath_mat <- file.path(base_path, "sample_counts.txt")
fpath_meta <- file.path(base_path, "sample_meta.txt")
lst_scrna <- mycytotalk::read_matrix_with_meta(fpath_mat, fpath_meta)
table(lst_scrna$cell_types)
```
...
# read in data folder
dir_in <- system.file("extdata", package = "mycytotalk")
lst_scrna <- mycytotalk::read_matrix_folder(dir_in)
